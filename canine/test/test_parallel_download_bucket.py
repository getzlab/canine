...
class TestReadAheadOverlapsReadAndWrite:
    """
    `io` reported 71–76% read on the bucket route with an overlap ceiling of 1.38x: the
    worker read a block, then wrote it, then read the next, so the network sat idle
    while GCS was being written and vice versa.

    §6.5g rejected this change for the pd-standard, where the ceiling was 1.05x and
    there was nothing to recover. That conclusion was route-specific and said so; this
    route is the other one.

    Deliberately a single-slot prefetch per chunk rather than a reader/writer pool. A
    pool would reorder writes, and two invariants forbid it: uploads within a GCS
    resumable session must be sequential, and `_hash` only records an S3 part it saw
    contiguously from its first byte. Both survive here because each chunk still writes
    in order on its own thread.
    """

    # The prefetch only engages when a chunk spans several blocks: the read-ahead is
    # submitted when `sent_to < end`. With the default 8 MiB upload block and a 1 MiB
    # chunk there is exactly one block per chunk and the feature is unreachable -- which
    # is how the first version of every test in this class passed while exercising
    # nothing. Each one therefore pins a small block against a larger chunk.
    BLOCK = 256 * 1024
    CHUNK = 2 * MIB

    def test_correctness_is_unchanged_with_and_without_it(self, tmp_path, monkeypatch,
                                                          gcs, payload, payload_md5):
        force_bucket_route(monkeypatch)
        for flag, name in ((True, "on.bam"), (False, "off.bam")):
            with Server(payload) as source:
                argv = options_for(str(tmp_path / name), source.url(), len(payload),
                                   check_md5=payload_md5,
                                   upload_block=self.BLOCK, min_chunk=self.CHUNK)
                argv.prefetch = flag
                assert pdl.run(argv) == pdl.EXIT_OK
            assert hashlib.md5(gcs.state.objects[OBJECT]).hexdigest() == payload_md5

    def test_only_one_read_is_ever_outstanding_per_stream(self, tmp_path, monkeypatch,
                                                          gcs, payload, payload_md5):
        """
        The safety property: two concurrent reads on one HTTP response would interleave
        bytes. The prefetch is sound only because the previous read has *returned*
        before the next is submitted.

        Third version of this test, because the first two measured the wrong thing.
        Counting reads observed *executing* makes the result depend on pool scheduling:
        at connections=1 the pool is single-threaded so two submitted reads simply
        queue, and at higher widths a saturated pool may serialise them anyway -- both
        pass with the invariant broken. The invariant is about SUBMISSION, so this
        counts submitted-but-not-finished reads per stream, incrementing at submit and
        decrementing when the read function returns. That is independent of how the pool
        chooses to run them.
        """
        peak = {"value": 0}
        live = {}
        lock = threading.Lock()
        real_executor = pdl.ThreadPoolExecutor

        class Tracking(real_executor):
            def submit(self, fn, *args, **kwargs):
                bound = getattr(fn, "__self__", None)
                if bound is None or getattr(fn, "__name__", "") != "read":
                    # Not a stream read. This class also intercepts the verification
                    # pool, whose plain functions have no __self__ -- keying those on
                    # id(None) put every one of them in the same bucket and reported 7
                    # outstanding on correct code.
                    return real_executor.submit(self, fn, *args, **kwargs)
                owner = id(bound)
                ...
                with lock:
                    ...
                    peak["value"] = max(peak["value"], live[owner])
                return real_executor.submit(self, counted, *args, **kwargs)

        monkeypatch.setattr(pdl, "ThreadPoolExecutor", Tracking)
        force_bucket_route(monkeypatch)
        with Server(payload) as source:
            assert pdl.run(options_for(
                str(tmp_path / "a.bam"), source.url(), len(payload),
                check_md5=payload_md5, connections=4,
                upload_block=self.BLOCK, min_chunk=self.CHUNK)) == pdl.EXIT_OK

        assert peak["value"] == 1, (
            "{} reads outstanding at once on one stream (expected exactly 1, and 0 "
            "would mean the prefetch never ran)".format(peak["value"]))

    def test_the_multipart_etag_still_verifies_from_recorded_digests(
            self, tmp_path, monkeypatch, gcs, payload, capsys):
        """
        _hash runs inside sink.write(), which prefetch reorders relative to reads but
        not relative to other writes. If that ever stopped holding, parts would go
        unrecorded and the ETag would fall back to reading them back.
        """
        force_bucket_route(monkeypatch)
        part = MIB
        parts = [payload[i:i + part] for i in range(0, len(payload), part)]
        etag = "{}-{}".format(
            hashlib.md5(b"".join(hashlib.md5(p).digest() for p in parts)).hexdigest(),
            len(parts))

        with Server(payload) as source:
            rc = pdl.run(options_for(str(tmp_path / "a.bam"), source.url(),
                                     len(payload), check_etag=etag,
                                     part_length=part, min_chunk=2 * MIB,
                                     upload_block=self.BLOCK))

        assert rc == pdl.EXIT_OK
        assert "0 re-read" in capsys.readouterr().err

    def test_a_rewind_discards_the_prefetched_buffer(self, tmp_path, monkeypatch, gcs,
                                                     payload, payload_md5):
        """
        When the sink persists less than was sent, the chunk rewinds and re-opens the
        source. A read issued for the old offset is in flight at that moment; it must be
        drained and its bytes thrown away, or the resumed stream would be written at the
        wrong place.
        """
        force_bucket_route(monkeypatch)
        dest = str(tmp_path / "sample.bam")
