...
**The decode reports its own split**, which is what decides whether that 5× is worth
attacking and how. Measured 2026-09-23 at two sizes:

```
small  170402482 -> 328888890, 1 member,  21 blocks
  k9pdl-gunzip read 2.420s inflate 2.047s write 6.145s  ceiling 1.73x  pipe2 1.73x
big    852012410 -> 1644444450, 5 members, 102 blocks
  k9pdl-gunzip read 26.080s inflate 10.135s write 29.077s  ceiling 2.25x  pipe2 1.80x
```
...
and was reverted (§13.53).

**This loop is not shaped like the relay.** At the big size the split is write 45% / read
40% / inflate 16% — genuinely balanced, not dominated. So unlike the relay, the answer
here is that pipelining has something to work with:

| | ceiling | implies | pipe2 | implies |
|---|---|---|---|---|
| small (170 MB) | 1.73× | 10.6 s → 6.1 s | 1.73× | 10.6 s → 6.1 s |
| big (852 MB) | 2.25× | 65.3 s → 29.0 s | 1.80× | 65.3 s → 36.3 s |

At the small size `pipe2 == ceiling`, because `read + inflate` (4.5 s) is still under
`write` (6.1 s) — the two-thread split is the whole prize and a third thread would idle.
At the big size `read + inflate` (36.2 s) overtakes `write` (29.1 s), so the third thread
pays: 2.25× against 1.80×.

**Do not read that as "three threads scale better with size."** Per byte, only one stage
moved:
...
| What is the decode's bottleneck now? | `gunzip-e2e-w*.json` | **fixed too.** `--decode-upload-width` (default 4) writes through 4 concurrent sessions and composes: write **31.1/30.6 s → 10.0/9.1 s**, `routeb` **73.6/74.4 → 56.1/54.4 s**. Width 8 buys 1.5% because the upload is now under the inflater's 155 MB/s, which cannot be parallelised. Cumulative with the read-ahead: **102.9 s → 54.4 s, 1.89×** |
| What does the sliced upload cost? | `gunzip-e2e-w4.json` | **the output becomes a 50-component composite with no md5Hash**, where width 1 leaves a plain object that has one. Nothing on this route verifies it — the advertised digest covers the compressed bytes — but `--decode-upload-width 1` is the way back |
| Biggest remaining cost in a `--gunzip` run? | `gunzip-e2e-verify-*.json` | **was the md5 verify read-back; fixed.** Same read-ahead, now also wrapped in a `phase()` so it stops being unattributed: **22.0/26.6 s → 6.1/6.7 s, 3.8×** (852 MB at ~133 MB/s against ~35 — essentially the 140.3 MB/s depth-4 ceiling). `routeb` **55.2 s → 36.1 s**; cumulative across all three changes **102.9 s → 36.1 s, 2.85×** |
| What is left in a `--gunzip` run? | `gunzip-e2e-verify-ra4.json` | at 1.64 GB: relay 5.4 s, verify 6.1 s, **decode 21.4 s**, other 2.8 s |
| Zero-copy slicing + queue slots? | `gunzip-zerocopy-*.json` | **shipped, 126.0 → ~98 s (1.28×)** on the VCF fixture. Zero-copy alone is 1.23× and speeds up *both* stages (inflate 58.8 → 49.8, write 55.1 → 40.3), confirming the copies were what sank §13.64. `queue_slices` adds a consistent 4%; default 8 (384 MiB) not 16 (640 MiB). Width 8 is inside the noise, width 16 worse — width stays 4 |
| Why not the 1.94× ceiling? | `gunzip-zerocopy-*.json` | **uploads are rate-limited, not serialised.** ~104 MB/s in situ against inflation's 183, so the queue drains slower than it fills. Fourth time the ceiling has overestimated here (§13.53, §13.60, §13.62, §13.68) — on this route it has never predicted an outcome |
| **What does a realistic VCF look like?** | `gunzip-e2e-vcf-ratio-*.json` | **the ratio matters far more than the size.** At **16.49:1** (552 MB → 9.11 GB, gnomAD-shaped) the **decode is 91% of the run** — relay 5.3 s, verify 3.1 s, decode 114.1 s — because relay/verify scale with the *compressed* size. inflate 58.8 s vs write 55.1 s, **ceiling 1.94×**: the best possible shape for a 2-thread split, worth 126 s → ~71 s. The 1.93:1 fixture used everywhere else is unrepresentative and was chosen to control PUT count, not ratio |
| **Does any of that hold at scale?** | `gunzip-e2e-10x-*.json` | **total is linear (10.01×), composition is not.** At 10× (8.5 → 16.4 GB): relay **14.17×** (155 → 110 MB/s per byte), verify **11.84×** (133 → 113), inflate 10.19×, write **8.20×** (improves — session setup amortizes), read 1.17× (fixed). Quote no per-byte figure without saying which size it came from |
| Does a 2-thread inflate ∥ upload split help? | `gunzip-split-attempt-q*.json` | **no — built, measured, reverted.** Predicted 21.4 → ~12 s; measured 21.25 → 20.4 s (~4%) and nothing in total, inside a 2.76 s rep-to-rep spread. Write fell 9.7 → 3.7 s as designed but **inflate rose 11.3 → 16.2 s**. Third time the overlap ceiling has overestimated here — it bounds the prize, it does not estimate it |
| Was that slowdown the GIL? Would multiprocessing fix it? | `gilprobe.py` | **no, and no.** Inflate under 4 upload workers: 12.74 s in threads, 12.67 s in **processes**, against 12.21 s solo — identical, so the GIL is not the mechanism and multiprocessing buys nothing. It also shows uploads cost inflate only **4%**, not 43%, so the split's slowdown came from its own buffering (copies through `bytes()` and a chunk-counted queue), not from overlap. The split is therefore unproven rather than disproven — a zero-copy retry could plausibly reach ~15 s |
