...
| Biggest remaining cost in a `--gunzip` run? | `gunzip-e2e-w4.json` | **the md5 verify read-back, ~26 s of 54 s.** It is a full sequential cold read of the sidecar and is not wrapped in a `phase()`. The read-ahead that fixed the decode's read has not been applied to it. Unmeasured |
