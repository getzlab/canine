...
| What is left in a `--gunzip` run? | `gunzip-e2e-verify-ra4.json` | relay 5.4 s, verify 6.1 s, **decode 21.4 s**, other 2.8 s. The decode is now inflate 11.4 s + upload 9.7 s, both near their floors — gzip inflation cannot be parallelised, and widening the upload past 4 buys 1.5%. A 2-thread inflate ∥ upload split is the only remaining structural win, worth ~21 s → ~12 s |
