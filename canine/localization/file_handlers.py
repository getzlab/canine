...
class HandleGSURL(FileType):
    ...
    def localization_command(self, dest):
        ...
        return f'[ ! -d {dest_dir}/{dest_file if self.is_dir else ""} ] && mkdir -p {dest_dir}/{dest_file if self.is_dir else ""} || :; gsutil {self.rp_string} -o "GSUtil:state_dir={dest_dir}/.gsutil_state_dir" cp -r -n -L "{dest_dir}/.gsutil_manifest" {self.path} {dest_dir}/{dest_file}'
