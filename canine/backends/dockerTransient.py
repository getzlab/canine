...
class DockerTransientImageSlurmBackend(TransientImageSlurmBackend): # {{{
    def __init__(
        ...
        image_family = "n4-h-c3-hml"
        image_project = "broad-tcga-wgs-thca-pran-7"
        if user is None:
    ...
    def init_slurm(self):
        ...
        try:
            image = self.dkr.images.get(f'gcr.io/{self.config["image_project"]}/slurm_gcp_docker_c3_hml:latest')
